...
function divIcon(e){
  ...
  return '<img class="ms-div" src="assets/'+(asc?'af-icono.png':'sf-icono.png')+'" alt="'+(asc?'Ascenso Frontier':'Superliga Frontier')+'" title="'+(asc?'Ascenso Frontier':'Superliga Frontier')+'" loading="lazy">';
}
function compBadge(comp){
  if(comp==='ascenso') return '<span class="badge badge-ascenso">Ascenso</span>';
  if(comp==='copa') return '<span class="badge badge-copa">Copa</span>';
  return '<span class="badge badge-superliga">Superliga</span>';
}
...
document.addEventListener('DOMContentLoaded', function(){
  fetch('datos_oficiales.json?t='+Date.now())
    ...
    .then(function(d){ bd = Object.assign(bd, d); window.bd = bd; renderAll(); })
    .catch(function(e){
...
function renderMatches(){
  ...
  $('matches').innerHTML = list.map(function(p){
    ...
    return '<article class="card spotlight match" data-comp="'+curComp+'" data-idx="'+i+'">'+
      ...
      '<div class="match-foot"><span>'+(p.fase?esc(p.fase):(T('jornada','Jornada')+' '+esc(p.jornada||'')))+'</span><span>'+(pen?'Pendiente':'Finalizado')+'</span></div>'+
    '</article>';
...
function scorerRow(r,i,comp){
  ...
  return '<div class="sc-row '+cls+'"'+(r.e&&r.j?' data-team="'+esc(r.e.id)+'" data-player="'+esc(encodeURIComponent(r.j.nombre))+'"':'')+'>'+
    ...
    '<div class="sc-who">'+
      ...
      (pos?'<span class="sc-pos">'+esc(pos)+'</span>':'')+
    ...
    '<header class="tm-hero">'+
      ...
      '<div class="tm-hero-in">'+
        ...
        '<div><h1 class="tm-name">'+esc(X(e.nombre))+'</h1>'+
          '<div class="tm-sub">'+(e.division==='SUPERLIGA'?'<span class="badge badge-superliga">Superliga Frontier</span>':'<span class="badge badge-ascenso">Ascenso Frontier</span>')+
...
function openPlayer(teamId,nameEnc){
  ...
  var hist=(j.historial||[]).map(function(h,hi){
    ...
    var divTxt=h.division==='ASCENSO'?'Ascenso Frontier':(h.division==='SUPERLIGA'?'Superliga Frontier':(h.division||'·'));
...
function openMatch(comp,idx){
      ...
      if(q.length>=3&&q[0].trim()){
        ...
        evs.push({min:parseInt(min)||0,html:
          '<div class="ev"'+(j&&f.e?' data-team="'+esc(f.e.id)+'" data-player="'+esc(encodeURIComponent(j.nombre))+'"':'')+'>'+
            ...
            (j&&j.posicion?'<span class="ev-pos">'+esc(j.posicion)+'</span>':'')+
  ...
  function teamCol(t,name,side){
    ...
    return '<div class="mt-team mt-team-'+side+'"'+(t?' data-team="'+esc(t.id)+'"':'')+'>'+
      ...
      '<b>'+esc(X(name))+'</b>'+
      (comp==='copa'?'<span class="mt-divi">'+divIcon(t)+(t?(t.division==='ASCENSO'?'Ascenso Frontier':'Superliga Frontier'):'')+'</span>':'')+
    '</div>';
  }
  ...
  $('sheet-match-body').innerHTML=
    '<header class="mt-hero">'+
      ...
      '<div class="mt-hero-in">'+
        '<div style="display:flex;gap:.5rem;justify-content:center;flex-wrap:wrap">'+(d?'<span class="badge '+d.c+'">'+d.t+'</span>':'')+compBadge(comp)+'<span class="badge">'+(p.fase?esc(p.fase):T('jornada','Jornada')+' '+esc(p.jornada||''))+'</span></div>'+
        '<div class="mt-teams">'+
          teamCol(L,p.local,'l')+
...
function palmares(idx){
  ...
  var out=[];
  var s=champ('SUPERLIGA'); if(s) out.push({comp:'Superliga Frontier',cls:'badge-superliga',e:s});
  var a=champ('ASCENSO');   if(a) out.push({comp:'Ascenso Frontier',cls:'badge-ascenso',e:a});
  ...
  if(fin){
    ...
    if(ce) out.push({comp:'Copa Fútbol Frontier',cls:'badge-copa',e:ce,marcador:fin.local+' '+gl(fin)+'-'+gv(fin)+' '+fin.visitante});
  }
...
var QUOTES=[
  {t:'Llevo tres ligas distintas probadas y esta es la única donde perder no se siente como una excusa de mala suerte del rival.',a:'Manager de Superliga Frontier',s:'Contenido de ejemplo',i:'assets/franshu.png',n:9.2},
  {t:'El tope salarial cambia la mentalidad completamente. No puedes fichar la solución fácil: tienes que jugar mejor.',a:'Manager de Ascenso Frontier',s:'Contenido de ejemplo',i:'assets/gabrii.png',n:8.7},
  {t:'Dejé Victory Road hace meses. Volví solo por esta liga y no me arrepiento.',a:'Ex-jugador reclutado',s:'Contenido de ejemplo',i:'assets/totti-alcresise.png',n:9.5}
];
function renderQuotes(){
  $('quotes').innerHTML=QUOTES.map(function(q,i){
    ...
    return '<blockquote class="card spotlight quote rv rv-d'+(i+1)+'">'+
      ...
      '<footer><img src="'+q.i+'" alt="" loading="lazy"><div><b>'+esc(q.a)+'</b><span>'+esc(q.s)+'</span></div></footer>'+
    '</blockquote>';
  }).join('');
}
...
async function shareMatch(comp,idx){
  ...
  var compTxt=comp==='ascenso'?'ASCENSO FRONTIER':comp==='copa'?'COPA FÚTBOL FRONTIER':'SUPERLIGA FRONTIER';
  var compCol=comp==='ascenso'?'#3E7BFF':comp==='copa'?'#FF3B3B':'#FF5100';
  var sub=(p.fase?p.fase:'JORNADA '+(p.jornada||'')).toUpperCase();
...
async function sharePlayer(teamId,nameEnc){
  ...
  ctx.fillText('SUPERLIGA FRONTIER',56,72);
  var divTxt=e.division==='ASCENSO'?'ASCENSO':'SUPERLIGA';
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(matchMedia('(hover:hover)').matches){
    ...
    var cur=$('cur'), cx=0,cy=0,tx=0,ty=0;
    ...
    document.addEventListener('mouseover',function(e){ if(e.target.closest&&e.target.closest('a,button,.card,tr,.squad-row,.sc-row,.br-match')) cur.classList.add('on'); });
    document.addEventListener('mouseout',function(e){ if(e.target.closest&&e.target.closest('a,button,.card,tr,.squad-row,.sc-row,.br-match')) cur.classList.remove('on'); });
  }
  ...
  (function musicaAmbiente(){
    var VOL = 0.10;                      // 10%: dentro del 8-12% pedido
    var boton = $('btn-sound');
    var audio = $('sf-audio');
